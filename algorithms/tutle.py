...
def turtle_trading(financial_data, window_size):
    ...
    signals['high'] = financial_data['trade_price'].shift(1).\
        rolling(window=window_size).max()
    # window_size-days low
    signals['low'] = financial_data['trade_price'].shift(1).\
        rolling(window=window_size).min()
    # window_size-days mean
    signals['avg'] = financial_data['trade_price'].shift(1).\
    ...
    signals['long_entry'] = financial_data['trade_price'] > signals.high
    signals['short_entry'] = financial_data['trade_price'] < signals.low

    #exit rule : the stock price crosses the mean of past window_size days.

    signals['long_exit'] = financial_data['trade_price'] < signals.avg
    signals['short_exit'] = financial_data['trade_price'] > signals.avg
...
def plot_tutle(goog_data):
    ...
    ax1.plot(ts.loc[ts.orders== 1.0].index,
            goog_data["trade_price"][ts.orders == 1.0],
